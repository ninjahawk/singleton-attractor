...
The $\beta$-threshold crossing produces $12.4\times 10^6$ times more
separation than flat-$\beta$ dynamics at the same time point.
...
Separate resource niches are not a sufficient failure condition, contrary to
Bostrom (2005): the real requirement is structurally different $\beta$-regimes.
\end{abstract}
...
\]
If $\rho(t) \to L < \infty$ with $L > 1 > \rho(0)$... but $\rho(0) > 1$
by A5, so $L > \rho(0) > 1$.
...
\begin{remark}
$\beta(S)$ is a sigmoid (Definition~2.3), so $\beta_1(S_1)$ varies as $S_1$
grows.
Throughout the proof, $|\beta_1|$ denotes $\inf_{S > T}|\beta(S)| > 0$,
the sigmoid's asymptotic magnitude for $S \gg T$.
Using this lower bound on $|\beta_1|$ gives an upper bound on $\tstar$;
the actual blowup is no later than computed.
\end{remark}
...
\end{lemma}
\begin{proof}
By Lemma~\ref{lem:rmin}, $r_1(t) > 1/2$ for all $t \geq \tcross$.
With $\beta_1 < 0$:
\[
...
Integrating:
\[
  u(t) < S_1(\tcross)^{-|\beta_1|} - \tfrac{|\beta_1|}{2}\,(t - \tcross).
\]
$u(t)$ reaches zero by time
\[
  \tstar = \tcross + \frac{2\,S_1(\tcross)^{-|\beta_1|}}{|\beta_1|} < \infty.
\]
...
\textbf{Explicit bound} (using tighter $r_1(\tcross)$ value):
\[
  \tstar \leq \tcross + \frac{T^{-|\beta_1|}}{|\beta_1|\,r_1(\tcross)},
  \quad r_1(\tcross) = \frac{T^\alpha}{T^\alpha + S_2(\tcross)^\alpha} > \tfrac{1}{2}.
\]
The conservative bound uses $r_1 > 1/2$; the explicit formula uses the
tighter value $r_1(\tcross)$ at threshold crossing.

\subsection{Step 3: Resource monopoly stability}
\label{sec:step3}

\begin{proposition}[Resource monopoly, local stability]
\label{prop:monopoly}
As $\rho \to \infty$, $R_1^*/\Rmax \to 1$ and $R_2^*/\Rmax \to 0$.
The equal-resource allocation ($S_1 = S_2$) is a \emph{locally} unstable
equilibrium: any small perturbation grows.
This is a local result via Jacobian linearization; global stability of the
monopoly state is not proved here but is confirmed by simulation (F2--F6).
\end{proposition}

\begin{proof}
From A2: $R_1^* = \Rmax/(1 + \rho^{-\alpha}) \to \Rmax$ as $\rho \to \infty$. \checkmark

For local instability: compute the Jacobian of the full 2D system at the
symmetric fixed point $S_1 = S_2 = S^*$.
Under A2, $dS_i/dt = S_i^\gamma / D$ where $\gamma = 1 - \beta + \alpha$
and $D = S_1^\alpha + S_2^\alpha$.
Let $C = S^{*(\gamma - \alpha - 1)}/4 > 0$.
Differentiating:
\[
  J = \begin{pmatrix} C(2\gamma - \alpha) & -C\alpha \\ -C\alpha & C(2\gamma - \alpha) \end{pmatrix}.
\]
This symmetric matrix has eigenvalues
\[
  \lambda_1 = 2C(\gamma - \alpha) = 2C(1-\beta),
  \qquad
  \lambda_2 = 2C\gamma = 2C(1-\beta+\alpha),
\]
with eigenvectors $[1,1]$ (uniform growth) and $[1,-1]$ (symmetry-breaking).
Since $\alpha > 0$ and $\beta < \alpha$ (pre-threshold), $\gamma > 0$,
so $\lambda_2 = 2C\gamma > 0$ always.
The $[1,-1]$ perturbation mode grows exponentially: the symmetric fixed
point is locally unstable in the symmetry-breaking direction.
Under A5, the system starts with $S_1 > S_2$, i.e., away from the fixed
point in the $[1,-1]$ direction.
\end{proof}
