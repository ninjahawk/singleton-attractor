...
singleton formation (zero formation failures in 300 trials per noise level);
cooperation under any structure---no cooperation, oracle-optimal cooperation,
rational cooperation---produces zero singleton-formation failures with
indistinguishable timing; and the timescale formula is
...
Section~\ref{sec:cosmology} discusses speculative cosmological implications
(not a formal result).
...
For the finite-time blowup result specifically: the proof uses only
$r_1(t) > 1/2$, which holds as long as $S_1 > S_2$ (Lemma~\ref{lem:rmin}),
independently of the equilibration timescale.
Even with arbitrarily slow resource adjustment, the post-threshold incumbent
holds more than half the resources at every moment and the blowup is
guaranteed.
...
If they could credibly commit to full concentration, the coalition would
function as a merged agent---a different scenario addressed in
Section~\ref{sec:failures} (F5: cooperation).
...
\textbf{Warning:} This section is informal speculation, not a result of the
formal model.
The model has no spatial structure (acknowledged in Section~\ref{sec:discussion}),
no speed-of-light constraint, and no mechanism for territorial expansion.
Assumption~A3 (fixed $\Rmax$) directly contradicts a cosmological setting where
accessible matter depends on expansion speed and the age of the universe.
The physical-unit mappings below are proposed heuristics, not derivations.
Readers assessing the paper's formal contributions should treat this section
as a separate, exploratory discussion.

The model is dimensionless.
Proposed mapping to physical quantities ($S$ = total computational substrate
controlled; $R$ = energy and matter) is not derived from the model but offered
as a conceptual bridge.
$\alpha \approx 1$ (linear coupling) is a first approximation;
$\alpha \approx 3$ would apply for volumetric 3D expansion.
Time-unit calibration: AI development scale, 1 unit $\approx$ 10--30 years;
cosmological scale, 1 unit $\approx$ 1--100 Myr.
