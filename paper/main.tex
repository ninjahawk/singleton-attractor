...
We prove three results formally and state one simulation-supported conjecture:
...
simultaneous-dynamics gap acknowledged).
...
\item \textbf{Three formal proofs and one simulation-supported conjecture}
...
dynamics (Theorem~\ref{thm:finitetime});
local instability of the equal-resource state
(Proposition~\ref{prop:monopoly}, local only);
...
For $N > 2$ it relies on Theorem~\ref{thm:nagent}, which carries an
unresolved simultaneous-dynamics gap (see that theorem's note).
The $N > 2$ case should be treated as a conjecture with strong simulation
support.
\end{theorem}
...
\begin{theorem}[$N$-agent singleton (simulation-supported)]
\label{thm:nagent}
...
No singleton-formation failures were observed in 300 trials at any tested
noise level ($\sigma$ from 0.001 to 1.0, F13).
Theorem~\ref{thm:stochastic} proves emergence probability~1; the simulation
confirms zero observed failures but cannot establish a probability.
