...
proxy.
On the formal side: three results are proved
...
simultaneous-dynamics gap; an explicit Dufresne-perpetuity formula
...
tending to $1$ as $\sigma \to 0$; the exact transcendental condition for
coalition external suppression yields $\alpha^* \approx 0.64$
(Theorem~\ref{thm:criticalalpha}).
...
public debate over takeoff speed---whether capability growth is currently
...
Related empirical work: \citet{ge2026exponential} compare exponential,
hyperbolic, and sigmoid models of capability growth on the METR
time-horizon benchmark using Bayesian model selection, with no
explicit super-exponential alternative; their methodology and dataset
are complementary to the calibration here.
...
The closest adjacent formal treatment is
\citet{anbarjafari2025singularity}, which establishes finite-time blow-up
criteria via Osgood-style conditions on a single-agent capability ODE
$\ddot{I} \geq aI^p$ and analyses physical and economic caps on
runaway dynamics; that framework does not model multi-agent competition,
coalition dynamics, or stochastic noise, and does not perform an
empirical calibration.
The present paper proves the $\beta$-flip survives adversarial resource
dynamics, which is non-trivial---an opponent can slow the race to
threshold, but not stop it.
