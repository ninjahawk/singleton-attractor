...
against frontier-AI training compute (\citealt{epoch2024compute}); the
result is reported alongside its limitations.
\end{abstract}
...
\item \textbf{An empirical calibration to frontier-AI training compute}
(Section~\ref{sec:calibration}) using the Epoch AI dataset
\citep{epoch2024compute}.
The frontier trajectory 2012--2026 is consistent with pure exponential
growth ($\gamma = +0.005$, 95\% CI $[-0.010,\;+0.021]$); the
2022--2026 sub-window shows mild deceleration.
Under this proxy the data do not support $\beta(S) < 0$.
\end{enumerate}
...
Appendix~\ref{sec:cosmology} is speculative context only.
...
\emph{Status:} This theorem is fully proved for $N = 2$
(Theorems~\ref{thm:ratio} and~\ref{thm:finitetime}).
For $N > 2$ it relies on Conjecture~\ref{thm:nagent}, which carries an
unresolved simultaneous-dynamics gap.
The $N > 2$ case is a conjecture, not a theorem.
\end{theorem}

The proof proceeds in four steps: Theorem~\ref{thm:ratio},
Theorem~\ref{thm:finitetime}, Proposition~\ref{prop:monopoly}, and
Conjecture~\ref{thm:nagent}.
...
the transverse perturbation mode grows exponentially at rate
$(\alpha - \beta)/(2 S^{\beta})$, so any initial heterogeneity (A5) is
amplified.
\end{proposition}
...
\[
  ...
  = \frac{\alpha - \beta}{2\,S_2^{\beta}}\,\varepsilon + O(\varepsilon^2),
\]
so $\varepsilon$ grows exponentially while $\alpha > \beta$.
...
Theorem~\ref{thm:ratio}, so the same Case~1/Case~2 argument gives
$\rho_{1N} \to \infty$.
Therefore $r_N = S_N^\alpha / \sum_j S_j^\alpha \leq \rho_{1N}^{-\alpha} \to 0$:
agent~$N$'s growth stalls.
The remaining $N-1$ agents $\{S_1, \ldots, S_{N-1}\}$ satisfy the inductive
hypothesis.
...
\subsection{Timescale formula (F17, empirical scaling estimate)}

\textit{The formula below is a fitted scaling estimate, not a derived result.
Only the $N^1$ exponent has an analytical derivation (Appendix~\ref{app:timescale}).
All other exponents are fitted from single-variable sweeps with no
cross-validation, no confidence intervals, and no accounting for interaction
effects.
It should not be used for quantitative prediction outside the tested parameter
range.}
...
the others are fitted from single-variable sweeps with all other parameters
held fixed.
The formula does not account for interaction effects between parameters and
has not been cross-validated on a held-out parameter set.
Confidence intervals on the fitted exponents are not reported; the formula
should be treated as a scaling estimate, not a precise prediction.
...
\begin{tabular}{lrrrl}
\toprule
Window & $n$ & $\gamma$ (curvature) & 95\% CI on $\gamma$ & Reading \\
\midrule
2010--2018 & 9  & $+0.157$ & $[-0.095,\;+0.409]$ & accelerating, CI straddles 0 \\
2018--2022 & 10 & $+0.082$ & $[-0.154,\;+0.317]$ & accelerating, CI straddles 0 \\
2022--2026 & 7  & $-0.069$ & $[-0.156,\;+0.018]$ & decelerating, CI nearly excludes 0 \\
2012--2026 & 26 & $+0.005$ & $[-0.010,\;+0.021]$ & exponential, CI tight around 0 \\
\bottomrule
\end{tabular}
\caption{Polynomial-in-time fits of frontier $\log_{10}$ training
compute. Curvature $\gamma$ is the coefficient on $t^{2}$ in
$\log_{10} S = a + bt + \gamma t^{2}$.
Positive $\gamma$ corresponds to super-exponential growth ($\beta < 0$).
...
exponential growth: $\gamma = +0.005$ with a 95\% CI of
$[-0.010,\;+0.021]$ that tightly straddles zero, and $R^{2} = 0.96$.
The doubling time is approximately $0.5$ years and roughly stable.
Sub-windows show mild acceleration during 2010--2022 and mild
deceleration during 2022--2026, but in every sub-window the 95\%
confidence interval includes $\gamma = 0$ (exponential).
The power-law fit, which is noisier, gives $\beta = -0.014$ on the full
window with a 95\% CI of $[-0.062,\;+0.035]$.

\subsection{Reading}

Under the frontier-compute proxy, frontier capability has been growing
exponentially, not super-exponentially.
The data are not consistent with $\beta(S)$ having flipped negative
during the period observed.
The 2022--2026 window---which the AI-safety literature sometimes
identifies as a possible take-off period---in fact shows a doubling
time that has lengthened from roughly $4$ months to roughly $7$ months.

This does not refute A4.
Three reasons the result is weak as evidence against the framework:
(i)~training compute is one of several capability inputs and the
post-training, scaffolding, and inference-compute regimes that have
become important since $2024$ are not captured here;
(ii)~A4 requires only that $\beta < 0$ become reachable at some future
$T$, not that the trajectory be already in that regime; the data are
silent on the future;
(iii)~$26$ frontier records over $14$ years is a small sample; tighter
data, particularly with capability measured by benchmark performance,
might reverse the curvature sign.
Conversely, the result \emph{is} evidence that the often-stated
``compute is going superexponential'' claim is not supported by the
public record on training FLOP through 2026, and that a singleton
emerging on this proxy alone within the next several years would
require a sharp regime change rather than continuation of the current
trend.

The exercise's main contribution is methodological: it makes A4 a
falsifiable hypothesis on a specific dataset.
A future study could repeat this calibration with capability proxies
better suited to recent trends (effective compute, frontier benchmark
performance, agentic capability scores) and with explicit confidence
bands on whether $\beta < 0$ is consistent with the data at any
horizon.
...
Section~\ref{sec:calibration} introduces the first empirical test of A4
on a public dataset: under frontier training compute as a capability
proxy, $\beta(S) < 0$ is not currently supported, and the framework's
central premise becomes a falsifiable hypothesis with negative
...
comes from the leader's increasing resource share as it advances.
The $\alpha^{-0.30}$ exponent tracks the integrated resource-correction;
the $|\betaL|^{-0.31}$ exponent is a mixture of pre- and post-threshold phases
(Appendix~\ref{app:timescale}).
The $\mathrm{gap}^{-0.15}$ exponent is small because the threshold mechanism
...
Equation~\eqref{eq:timescale} is fitted from single-variable sweeps with no
cross-validation.
The $N^1$ exponent has an analytical derivation; the others do not.
Use it as a rough scaling guide, not a prediction.
...
\section{Cosmological Implications (Speculative)}
\label{sec:cosmology}

This appendix is informal speculation and is not a result of the formal model.
The model has no spatial structure, no speed-of-light constraint, and no
mechanism for territorial expansion.
Assumption~A3 (fixed $\Rmax$) directly contradicts a cosmological setting
where accessible matter depends on expansion speed and the age of the universe.
The time-unit calibration spans 3--4 orders of magnitude, making quantitative
constraints effectively unconstrained.
Take this as motivating context, not formal claims.
