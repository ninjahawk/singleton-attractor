...
suppression ($\alpha^* \approx 0.64$, Theorem~\ref{thm:criticalalpha}) and a
stochastic blowup proof via Feller's criterion (Theorem~\ref{thm:stochastic}).
Eleven simulation scripts verify the ODE system's numerical behavior across
25 parameter configurations.
Threshold-crossing dynamics produce arbitrarily large separation in finite
time (illustrated in Fig.~\ref{fig:threshold}).
Stochastic noise at all tested amplitudes randomizes which agent wins; it does
not prevent a singleton from forming.
Neither cooperation structure nor coalition size prevents singleton emergence
...
The theorems prove that \emph{if} A4 holds, a singleton emerges---and they
specify how: in finite time, under adversarial resource dynamics, robust to
...
A4---is outside the paper's reach.
...
Singleton emergence rate (ratio $> 10\times$): zero failures at every tested
level.
Winner identity: initial leader wins $> 50\%$ of trials below $\sigma \approx
0.23$ (default 10\% gap); degradation begins near $\sigma \approx 0.023$.}
\label{fig:stochastic}
\end{figure}

Theorem~\ref{thm:stochastic} proves singleton formation with probability~1
for any $\sigma > 0$; this is the actual proof of the result.
The simulation (300 trials per noise level, zero failures) is a consistency
check against the theorem, not independent evidence---it confirms the ODE
integrations behave as the SDE analysis predicts.
Noise affects \emph{which} agent wins, not \emph{whether} a singleton forms.
...
\begin{proof}
Let $|\beta^*| \in (0, |\betaL|)$ be the post-escape constant from
Lemma~\ref{lem:escape}, so $|\beta(S_1)| \geq |\beta^*|$ once $S_1 \geq T'$.
For $S_1 \geq T'$, the SDE~\eqref{eq:sde} gives, by It\^{o}'s formula applied
to $u = S_1^{-|\beta^*|}$:
\[
  du = \Bigl(-|\beta^*|\rmin
     ...
     - |\beta^*|\sigma\,u\,dW.
\]
This is a linear SDE in $u$ with constant negative term $-|\beta^*|\rmin < 0$.
Its explicit solution is
\[
  u(t) = M(t)^{-1}\!\left[u(t_1) - |\beta^*|\rmin \int_{t_1}^t M(s)\,ds\right],
\]
where
$M(s) = \exp\!\bigl(-\tfrac{|\beta^*|\sigma^2}{2}(s-t_1)
                    + |\beta^*|\sigma(W(s)-W(t_1))\bigr)$
is a geometric Brownian motion.
Since $\int_{t_1}^\infty M(s)\,ds = \infty$ a.s.\
(the integrand is positive and the exponent has positive quadratic variation
dominating the drift for large $|\beta^*|$; see the remark below),
there is a finite random time $\tstar > t_1$ at which the bracket vanishes:
$u(\tstar) = 0$, i.e.\ $S_1(\tstar) = \infty$.
Therefore $S_1 \to \infty$ in finite time a.s.
\end{proof}

\begin{remark}
Noise affects the \emph{distribution} of $\tstar$ but not the event
$\{\tstar < \infty\}$.
The explosion time $\tstar$ is the first time the integral
$\int_{t_1}^t M(s)\,ds$ reaches $u(t_1)/(|\beta^*|\rmin)$; this is finite
a.s.\ because $M(s) > 0$ always and the integral is non-decreasing and
unbounded.
\end{remark}

\begin{remark}[Consequence for agent~2]
Suppose agent~2 also satisfies a multiplicative-noise SDE
$dS_2 = r_2(t)\,S_2^{1-\beta_2}\,dt + \sigma\,S_2\,dW_2$
with $\beta_2 > 0$ and $W_2$ independent of $W$.
The drift is sublinear and the noise is GBM-type, so by standard non-explosion
criteria \citep[Prop.~5.5.32]{karatzas1991brownian} $S_2(t) < \infty$ a.s.\
for all finite $t$.
Combined with $S_1 \to \infty$ a.s.\ at finite $\tstar$
(Theorem~\ref{thm:stochastic}), $\rho = S_1/S_2 \to \infty$ a.s.\ at $\tstar$.
\end{remark}
...
On the stochastic side, Theorem~\ref{thm:stochastic} closes a gap that prior
informal arguments left open.
Feller's criterion gives a clean, complete proof that noise changes the winner
distribution but not the emergence probability.
...
Initial conditions matter less than threshold position; noise changes who wins,
not whether a winner emerges; cooperation under any tested structure fails to
prevent it.
...
Whether any system can actually achieve $\beta < 0$ is what the formal
results cannot address.
That is the gap between the mathematics and anything it might mean in
practice.
